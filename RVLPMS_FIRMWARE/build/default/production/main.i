...
uint8_t btn_state = 0;
uint32_t btn_time_pressed = 0;
uint8_t btn_high_edge = 1;
uint8_t btn_press_count = 0;
uint32_t btn_press_timer = 0;
uint8_t btn_long_edge = 0;
...
if(readADC(0b000100) >= 222) {
SYS_ENABLE = 0;
TRISCbits.TRISC5 = 1;
BQ_Write(0x09, 0b01100100);
}
}
...
pwr_btn_temp = RA5;
if(pwr_btn_temp ^ pwr_btn_temp_prev) {
...
if(btn_state == 0) {
if(btn_high_edge == 0) {
if(SYS_ENABLE && timer_diff(btn_time_pressed)<=50){
if(btn_press_count == 0) btn_press_timer = get_time();
btn_press_count++;
if(btn_press_count == 2) {
if(!VBUS_CHRG_STATE[1]) mode++;
}
}
}
btn_high_edge = 1;
...
if(timer_diff(btn_press_timer) > 50) btn_press_count = 0;
