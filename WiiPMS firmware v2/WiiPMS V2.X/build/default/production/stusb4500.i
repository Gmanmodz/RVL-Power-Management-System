...
if (ok) ok = i2c_master_write_u16(0x28, 0x51, msg);
if (ok) ok = i2c_master_write_u8(0x28, 0x1A, 0x26);
...
if (ok == 0)
ok = i2c_master_write_u8(0x28, 0x23, 0x01);
...
if (ok == 0) _delay((unsigned long)((27)*(32000000/4000.0)));


if (ok == 0)
ok = i2c_master_write_u8(0x28, 0x23, 0x00);
...
if (pdo_num < 1 || pdo_num > 3) return -1;


uint32_t pdo =
0x00000000 | ((current_mA / 10) & 0x03FF) | ((uint32_t)((voltage_mV / 50) & 0x03FF) << 10);


if (
i2c_master_write_u32(0x28, 0x85 + 4 * (pdo_num - 1), pdo) !=
0)
return -1;
...
uint16_t pdo_current = ((pdo & 0x03FF) * 10);
uint16_t pdo_voltage = (((pdo >> 10) & 0x03FF) * 50);
uint32_t pdo_power = (uint32_t)pdo_current * pdo_voltage / 1000;

if (
((pdo >> 30) & 0x03) != 0x00 || pdo_current < 0 ||
pdo_voltage < 5000 || pdo_voltage > 12000)
continue;
...
if (ok == 0) ok = write_pdo(opt_pdo_current, opt_pdo_voltage, 3);
...
if (
i2c_master_read_u8(0x28, 0x0E, buffer) != 0 ||
!(buffer[0] & 0x01))
return -1;
...
if (i2c_master_read_u8(0x28, 0x16, buffer) != 0)
return -1;


if (!(buffer[0] & 0x04)) continue;


if (i2c_master_read_u16(0x28, 0x31, &header) != 0)
return -1;
...
if (i2c_master_read_u8(0x28, 0x30, buffer) != 0)
return -1;


if (buffer[0] != ((header >> 12) & 0x07) * 4)
return -1;
...
(uint32_t*)buffer, ((header >> 12) & 0x07));
}
